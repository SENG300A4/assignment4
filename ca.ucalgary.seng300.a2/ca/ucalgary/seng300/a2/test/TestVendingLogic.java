...
public class TestVendingLogic {
	...
	public void setUp() throws Exception {
		...
		// Canadian coins, 6 types of pop, capacity of coinRack=15, 10 pops per
		// rack, 200 coins in receptacle,
		// 200 coins in delivery chute, 15 coins in coin return slot

		VendingMachine vendingMachine = new VendingMachine(canadianCoins, numPopTypes, coinRackCapacity, 15, 200, 200,
				15);
	...
	/**
	 * Tests if pop is dispensed correctly with valid coin insertions and button
	 * presses
	 */
	@Test
	public void testDispense() {
		...
		try {
			...
			// for (int i = 0; i < vendingMachine.getNumberOfSelectionButtons();
			// i++) {
	...
	/**
	 * Tests a typical user case
	 * Enough credit, button pressed, checks for correct pop and that
	 * the pop has been dispensed.
	...
	public void testReturnExactChangeOutOfCoins() {
		...
		// Since machine is out of toonies, two loonies should have been
		// returned in place of toonies
		vendingLogic.provideChange(vendingLogic.getCredit());
		assertEquals(0, vendingLogic.getCredit());
	...
	public void testReturnInexactChange() {
		...
		// initial number of loonies, quarters, dimes, nickels in respective
		// coin racks
		int num_loonies = vendingMachine.getCoinRackForCoinKind(100).size();
	...
	/**
	 * Verifies no pop is dispensed when there is insufficient credit
	 */
	@Test
	...
	/**
	 * Tests if valid debit purchases can be made.
	 * Valid card numbers/PINs are:
	 * 123/1234
	 * 321/4321
	 */
	@Test
	public void testValidDebitPurchase() {
		//Set the tender type to debit
		vendingLogic.setTenderType(2);
		vendingMachine.getSelectionButton(0).press();
		assertEquals(0, vendingMachine.getPopCanRack(0).size());
	}
	
	/**
	 * Tests if valid credit purchases can be made.
	 * Valid card numbers/PINs are:
	 * 123/1234
	 * 321/4321
	 */
	@Test
	public void testValidCreditPurchase() {
		//Set the tender type to credit
		vendingLogic.setTenderType(3);
		vendingMachine.getSelectionButton(0).press();
		assertEquals(0, vendingMachine.getPopCanRack(0).size());
	}
	
	/**
	 * Tests if invalid debit purchases can be made.
	 * Invalid purchases must be made on card 321 (for now), or with an invalid PIN or card number.
	 * Valid card numbers/PINs are:
	 * 123/1234
	 * 321/4321
	 */
	@Test
	public void testInvalidDebitPurchase() {
		//Store the current number of pop cans for assertion purposes
		int num_popCans = vendingMachine.getPopCanRack(0).size();
		//Set the tender type to debit
		vendingLogic.setTenderType(2);
		// Change the price of Cola to $10
		java.util.List<String> popCanNames = Arrays.asList("Cola", "Sprite", "Fonda", "Diet", "GingerAle", "DrPepper");
		java.util.List<Integer> popCanCosts = Arrays.asList(1000, 250, 250, 250, 250, 250);
		int[] popCanCounts = new int[vendingMachine.getNumberOfPopCanRacks()];
		for (int i = 0; i < popCanCounts.length; i++) {
			popCanCounts[i] = 1;
		}
		vendingMachine.configure(popCanNames, popCanCosts);
		vendingMachine.getSelectionButton(0).press();
		assertEquals(num_popCans, vendingMachine.getPopCanRack(0).size());
	}
	
	/**
	 * Tests if invalid credit purchases can be made.
	 * Invalid purchases must be made on card 321 (for now), or with an invalid PIN or card number.
	 * Valid card numbers/PINs are:
	 * 123/1234
	 * 321/4321
	 */
	@Test
	public void testInvalidCreditPurchase() {
		//Store the current number of pop cans for assertion purposes
		int num_popCans = vendingMachine.getPopCanRack(0).size();
		//Set the tender type to credit
		vendingLogic.setTenderType(3);
